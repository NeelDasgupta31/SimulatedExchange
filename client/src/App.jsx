...
function NameModal({ onConfirm }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.92)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 3000 }}>
      <div style={{ background: BG2, border: `1px solid ${BORDER}`, padding: 32, width: 320, textAlign: 'center' }}>
        <div style={{ color: BLUE, fontSize: 18, fontWeight: 'bold', marginBottom: 6 }}>SimExchange</div>
        ...
        <input
          ...
          maxLength={20}
          style={{ width: '100%', background: BG3, border: `1px solid ${BORDER}`, color: TEXT, padding: '8px 10px', fontSize: 13, boxSizing: 'border-box', marginBottom: 12, textAlign: 'center' }}
        />
        <button onClick={submit}
          style={{ width: '100%', background: BG3, border: `1px solid ${BLUE}`, color: BLUE, padding: '8px', cursor: 'pointer', fontSize: 12 }}>
          Join Game
        </button>
      </div>
    </div>
...
function ResultsModal({ markets, leaderboard, myPositions, traderId, totalPnl, onClose }) {
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2000 }}>
      <div style={{ background: BG2, border: `1px solid ${BORDER}`, padding: 28, width: 480, maxHeight: '85vh', overflowY: 'auto' }}>
        ...
        <div style={{ marginBottom: 16, background: BG3, border: `1px solid ${BORDER}`, padding: 12 }}>
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          {leaderboard.slice(0, 10).map((e, i) => (
            <div key={e.traderId} style={{ display: 'flex', justifyContent: 'space-between', padding: '3px 6px', fontSize: 11, background: e.traderId === traderId ? '#0d2a0d' : 'transparent', borderBottom: `1px solid ${BORDER}` }}>
        ...
        <button onClick={onClose} style={{ width: '100%', background: BG3, border: `1px solid ${BORDER}`, color: BLUE, padding: '8px', cursor: 'pointer', fontSize: 12 }}>
          Close
        </button>
      </div>
    </div>
...
export default function App() {
  ...
  const [playerName, setPlayerName] = useState(null); // null = not set yet
  const [showResults, setShowResults] = useState(false);
  const [resultsShown, setResultsShown] = useState(false);

  const { traderId, markets, books, myPositions, myTrades, activeOrders, totalPnl, recentTrades, news, leaderboard, connected, roundName, positionLimit } = state;
  ...
  return (
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', background: BG, color: TEXT, overflow: 'hidden', fontFamily: "'Segoe UI', Arial, sans-serif", fontSize: 11 }}>
      ...
      {/* Top bar */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '4px 14px', background: BG3, borderBottom: `1px solid ${BORDER}`, minHeight: 36, flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <span style={{ color: BLUE, fontWeight: 'bold', fontSize: 14, letterSpacing: 1 }}>SimExchange</span>
        ...
        <div style={{ display: 'flex', gap: 20, alignItems: 'center' }}>
          ...
          <div style={{ background: BG2, border: `1px solid ${BORDER}`, padding: '2px 12px', textAlign: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
          ...
          {allRevealed && (
            <button onClick={() => setShowResults(true)} style={{ background: '#1a1400', border: '1px solid #554400', color: GOLD, padding: '2px 10px', cursor: 'pointer', fontSize: 10 }}>Results</button>
          )}
        </div>
      </div>
          ...
          {markets.length === 0 ? (
            ...
            <div style={{ height: '100%', display: 'flex', overflowX: 'auto', overflowY: 'hidden' }}>
              {orderedMarkets.map(market => (
                <div key={market.id} style={{ width: 178, minWidth: 178, flexShrink: 0, borderRight: `1px solid ${BORDER}`, display: 'flex', flexDirection: 'column', background: BG }}>
                  <OrderBook
                    market={market}
      ...
      <div style={{ display: 'flex', borderTop: `1px solid ${BORDER}`, flexShrink: 0, height: 155, overflow: 'hidden' }}>
        ...
        {/* News */}
        <div style={{ flex: 1.2, overflow: 'hidden', borderLeft: `1px solid ${BORDER}`, display: 'flex', flexDirection: 'column' }}>
        ...
        {/* Leaderboard */}
        <div style={{ width: 155, display: 'flex', flexDirection: 'column', borderLeft: `1px solid ${BORDER}`, overflow: 'hidden' }}>
          ...
          <div style={{ flex: 1, overflowY: 'auto' }}>
            {leaderboard.map((e, i) => (
              <div key={e.traderId} style={{ padding: '2px 8px', fontSize: 9, borderBottom: `1px solid ${BG3}`, background: e.traderId === traderId ? '#0d1a0d' : 'transparent', display: 'flex', justifyContent: 'space-between' }}>
                <span style={{ color: i === 0 ? GOLD : i === 1 ? '#c0c0c0' : i === 2 ? '#cd7f32' : DIM, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 90 }}>
                  #{i+1} {e.traderId === traderId ? `${e.displayName} ★` : e.displayName}
                </span>
                <span style={{ color: e.pnl >= 0 ? GREEN : RED }}>{(e.pnl||0).toFixed(1)}</span>
              </div>
            ))}
          </div>
        </div>

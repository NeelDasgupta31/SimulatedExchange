...
app.post('/api/host/round/start', (req, res) => {
  ...
  const { markets, roundName, tickSize } = req.body;
  if (!markets || !markets.length) return res.status(400).json({ error: 'No markets provided' });
  exchange.stop();
  exchange.setupMarkets({ markets }, roundName || 'ROUND', Number(tickSize) || 0.1);
...
app.post('/api/host/settle', (req, res) => {
  if (!authHost(req, res)) return;
  const { trueValues } = req.body;
  if (!trueValues || typeof trueValues !== 'object') return res.status(400).json({ error: 'trueValues required' });
  exchange.settle(trueValues);
  res.json({ success: true });
});
